...
RAPIDO is a C++ framework designed to make writing HEP analyses more ergonomic and readable. It wraps some basic functionality of \href{https://root.cern/}{\texttt{ ROOT}}. The idea is that an analysis, in general, consists of a few objects\+: a TTree (to hold some skimmed N-\/\+Tuple and/or a set of histograms), a cutflow (a collection of boolean logic for filtering events), and a looper (some way to run over multiple files). RAPIDO is designed to handle all three of these tasks such that every analysis that uses it is structured in the same way. In addition, the {\itshape way} in which it is structured lends itself to the common workflow of a HEPEx-\/er.\hypertarget{md_README_autotoc_md5}{}\doxysection{RAPIDO Tools}\label{md_README_autotoc_md5}
...
\end{DoxyEnumerate}\hypertarget{md_README_autotoc_md6}{}\doxysection{Set Up Instructions}\label{md_README_autotoc_md6}
...
\begin{DoxyCode}{0}
\DoxyCodeLine{\$ g++ main.cc -\/o ./main -\/lRAPIDO -\/Lrapido -\/Irapido}
\DoxyCodeLine{\$ export LD\_LIBRARY\_PATH=\$LD\_LIBRARY\_PATH:\$PWD/rapido}
\DoxyCodeLine{\$ ./main}

\end{DoxyCode}

\end{DoxyEnumerate}\hypertarget{md_README_autotoc_md7}{}\doxysection{Examples}\label{md_README_autotoc_md7}
...
\DoxyCodeLine{myanalysis \$ mv MySelector.C MySelector.cc}
\DoxyCodeLine{myanalysis \$ rootcint myselectordict.cc -\/c MySelector.h}
\DoxyCodeLine{myanalysis \$ mv myselectordict* rapdio/}
\DoxyCodeLine{myanalysis \$ mv MySelector* rapido/}
\DoxyCodeLine{myanalysis \$ cd rapido/}
\DoxyCodeLine{myanalysis \$ make -\/j5}
